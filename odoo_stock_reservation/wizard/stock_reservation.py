                    ...
                    reserv_move_id = custome_reservtion_obj.create(
                        {
                            ...
                            "reserv_due_date": fields.Datetime.now()
                            + relativedelta(days=self.sale_order_id.reserve_days)
                            if self.sale_order_id.reserve_days
                            else False,
                        }
                    )
...
class StockReservationLine(models.TransientModel):
    ...
    order_line_id = fields.Many2one(
        ...
        required=True,
        ondelete="cascade",
        domain=lambda self: [("order_id", "=", self._context.get("current_sale_order_id"))],
    )
